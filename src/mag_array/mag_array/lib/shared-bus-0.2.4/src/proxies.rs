...
#[cfg(feature = "eh-alpha")]
use embedded_hal_alpha::spi as spi_alpha;
...
#[cfg(feature = "eh-alpha")]
impl<'a, M: crate::BusMutex> spi_alpha::ErrorType for SpiProxy<'a, M>
    where
        M::Bus: spi_alpha::ErrorType,
{
    type Error = <M::Bus as spi_alpha::ErrorType>::Error;
}

/// Flush doesn't do anything, because the mutex lock mechanism guarantees that transfers are fully completed
impl<'a, M: crate::BusMutex> spi_alpha::SpiBusFlush for SpiProxy<'a, M>
    where
        M::Bus: spi_alpha::SpiBusFlush,
{
...
#[cfg(feature = "eh-alpha")]
impl<'a, M: crate::BusMutex> spi_alpha::SpiBusWrite for SpiProxy<'a, M>
    where
        M::Bus: spi_alpha::SpiBusWrite,
{
    fn write(&mut self, words: &[u8]) -> Result<(), Self::Error> {
        self.mutex.lock(|bus| bus.write(words))
    }
...
#[cfg(feature = "eh-alpha")]
impl<'a, M: crate::BusMutex> spi_alpha::SpiBusRead for SpiProxy<'a, M>
    where
        M::Bus: spi_alpha::SpiBusRead,
{
    fn read(&mut self, words: &mut [u8]) -> Result<(), Self::Error> {
        self.mutex.lock(|bus| bus.read(words))
    }
}

#[cfg(feature = "eh-alpha")]
impl<'a, M: crate::BusMutex> spi_alpha::SpiBus for SpiProxy<'a, M>
    where
        M::Bus: spi_alpha::SpiBus,
{
    fn transfer(&mut self, read: &mut [u8], write: &[u8]) -> Result<(), Self::Error> {
        self.mutex.lock(|bus| bus.transfer(read, write))
    }

    fn transfer_in_place(&mut self, words: &mut [u8]) -> Result<(), Self::Error> {
        self.mutex.lock(|bus| bus.transfer_in_place(words))
    }
}
